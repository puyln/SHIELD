...
def main() -> None:
    parser = build_parser("Train the SHIELD virtual DCE-MRI generation module.")
    args = parser.parse_args()
    cfg = load_config(args.config, args.manifest)
    cases = validate_manifest(get_manifest(cfg, args.manifest))
    print(f"Validated {cases} manifest case(s) for generation.")
    if args.dry_run:
        print_config(cfg)
        return
    public_release_unavailable("Generation training")
